...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const data = await authRegister(username.trim(), password, inviteCode.trim());
    ...
    <div className="min-h-screen flex items-center justify-center px-4">
      <div className="w-full max-w-md animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="glass-surface rounded-2xl p-8 space-y-6">
          <h2 className="font-title text-xl text-slate-200 tracking-wide">注册</h2>
          ...
          <div className="space-y-1.5">
            <label htmlFor="reg-invite" className="text-xs text-slate-400 uppercase tracking-wider">
              邀请码
            </label>
            <input
              id="reg-invite"
          ...
          <p className="text-center text-sm text-slate-500">
            已有账号？{" "}
            <Link to="/login" className="text-qi hover:text-qi/80 transition-colors">
              登录
            </Link>
          </p>
        </form>
